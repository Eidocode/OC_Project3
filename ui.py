...
class Slot:
    """
        Instanciated in UI class. Contains the sprite slot depending on it is empty or not.
        There is slot size, position and state (empty or not)
    """
    def __init__(self):
        self.sprite_slot = pygame.image.load(const.sprite_placeholder).convert_alpha()
        self.empty_sprite_slot = pygame.image.load(const.sprite_placeholder).convert()
...
class UI:
    """
        This class represents User Interface. It contains the inventory slots where the items are
        displayed and the player and guardian states. There is also the special slot where the
        syringe is diplayed when the 3 items have been picked up.
    """
    ...
    def init_ui(self):
        ...
        for slot in range(self.nb_inv_slot):
            ...
            self.slot_pos_x += slot.size + 10 # Modify slot_pos_x for next slot

    @property
    def add_special_slot(self):
        """ Method used to create the special slot """
        slot = Slot()
        slot.sprite_slot = pygame.image.load(const.sprite_spec_placeholder).convert_alpha()
    ...
    def add_char_picture(self, sprite, x, y, lifebar_type):
        ...
        spr_lifebar_full = pygame.image.load(const.sprite_lifebar_full).convert_alpha()
        spr_lifebar_weak = pygame.image.load(const.sprite_lifebar_weak).convert_alpha()
        offset = 50
    ...
    def draw(self):
        ...
        if self.player.inventory.combine_items:
            self.add_icon_in_slot(const.sprite_seringue, special_slot)
        else:
            self.window.blit(special_slot.empty_sprite_slot, (special_slot.x, special_slot.y))

        # Normal slots stored in 'list_slot_ui'
        for slot in UI.list_slot_ui:
            self.window.blit(slot.sprite_slot, (slot.x, slot.y)) # Draw slot
            if len(self.player.inventory.content) > 0: # Check if inventory is not empty
                for item in self.player.inventory.content:
                    if slot.is_empty:
                        if not item.is_draw_ui:
                            slot.is_empty = False
                            item.is_draw_ui = True
                            self.add_icon_in_slot(item.ui_icon, slot) # Add item icon to UI slot
            else:
                self.window.blit(slot.empty_sprite_slot, (slot.x, slot.y)) # Add Empty slot
        ...
        if not self.player.is_weak:
            self.add_char_picture(const.sprite_ui_mcgyver, player_ui_pos_x, picture_ui_pos_y, 0)
            self.add_char_picture(const.sprite_ui_guard, guard_ui_pos_x, picture_ui_pos_y, 1)
        else:
            self.add_char_picture(const.sprite_ui_mcgyver, player_ui_pos_x, picture_ui_pos_y, 1)
            self.add_char_picture(const.sprite_ui_guard, guard_ui_pos_x, picture_ui_pos_y, 0)
